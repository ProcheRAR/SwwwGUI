...
class SwwwGuiWindow(Adw.ApplicationWindow):
    ...
    def build_ui(self):
        ...
        self.preview_button = Gtk.Button(label=self.application.translator.translate("preview"))
        self.preview_button.set_tooltip_text(self.application.translator.translate("preview"))
        ...
        flap_toggle = Gtk.Button()
        flap_toggle.set_icon_name("sidebar-show-symbolic")
        flap_toggle.connect("clicked", self.on_flap_toggle)
        header_bar.pack_end(flap_toggle)
    ...
    def on_flap_toggle(self, button):
        """Toggle the side panel."""
        self.content_flap.set_reveal_flap(not self.content_flap.get_reveal_flap())
    ...
    def on_preview_clicked(self, button):
        """Preview the current image with transition effects."""
        if not self.image_view.current_image_path:
            return
        ...
        # For preview, use a simpler transition without animations
        # to make it respond quickly
        preview_options = options.copy()
        preview_options['transition_type'] = 'none'  # Skip transition animation
        
        # Check if matugen is enabled
        use_matugen = self.config.get('use_matugen', False)
        
        if use_matugen:
            # Even for preview, use standard swww for quick response
            success = self.swww_manager.set_wallpaper(
                self.image_view.current_image_path,
                preview_options
            )
        else:
            # Apply wallpaper with preview options (fast response)
            success = self.swww_manager.set_wallpaper(
                self.image_view.current_image_path,
                preview_options
            )
        ...
        if success:
            toast = Adw.Toast.new(self.application.translator.translate("preview_applied"))
            toast.set_timeout(1)
            self.add_toast(toast)
        else:
            # Show error dialog
            error_dialog = Adw.MessageDialog(
                heading=self.application.translator.translate("error"),
                body=self.application.translator.translate("preview_failed"),
                close_response="ok"
            )
            error_dialog.add_response("ok", self.application.translator.translate("ok"))
            error_dialog.present()
    ...
    def _update_settings_pages(self):
        ...
        # Update matugen settings - use consistent approach for all titles with capitalized Matugen
        ...
        if hasattr(self, 'matugen_group'):
            self.matugen_group.set_title(tr.translate("matugen_integration").replace("matugen", "Matugen"))
            
        if hasattr(self, 'matugen_row'):
            self.matugen_row.set_title(tr.translate("use_matugen").replace("matugen", "Matugen"))
    ...
    def update_localization(self):
        ...
        # Update buttons
        self.settings_button.set_tooltip_text(self.application.translator.translate("settings"))
        self.preview_button.set_label(self.application.translator.translate("preview"))
        self.preview_button.set_tooltip_text(self.application.translator.translate("preview"))
